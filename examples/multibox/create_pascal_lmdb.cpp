...
DEFINE_string(label_filename, "",
              "The file that contains the PASCAL label annotations.");
DEFINE_string(lmdb_dir, "", "The directory that contains the lmdb files.");
DEFINE_int32(width, 224, "The new width of the image.");
DEFINE_int32(height, 224, "The new height of the image.");
...
int main(int argc, char **argv) {
  ...
  while (label_file >> image_filename) {
    ...
    cv::Mat image = caffe::ReadImageToCVMat(image_filename, FLAGS_height,
                                            FLAGS_width, kIsColor);
    ...
    int num_pixels = 3 * FLAGS_width * FLAGS_height;
